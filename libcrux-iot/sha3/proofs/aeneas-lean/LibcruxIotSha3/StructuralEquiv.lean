...
-- TODO(new-aeneas): the post-`hax_mvcgen` hypothesis layout has changed;
-- `rename_i r hst hi_eq hd hc` no longer matches. Stubbed.
local macro "round1_theta_c_proof" _subfun:ident : tactic => `(tactic| sorry)
...
/- Local copy of `Foundation.PrcLift`'s `prc_y_zeta_no_rc_proof` macro
   ...
   `hax_mvcgen` resolve. -/
-- TODO(new-aeneas): hypothesis names from `expose_names` no longer match.
-- Stubbed.
local macro "round1_prc_y_zeta_no_rc_proof" _subfun:ident : tactic => `(tactic| sorry)
...
private theorem round1_pi_rho_chi_y0_zeta0_spec_fc
    ...
    ⦃ ⇓ r => ⌜
      ...
      r.st.val = apply_5_writes s.st.val
        ...
        (bx3 ^^^ ((~~~bx4) &&& bx0))
        (bx4 ^^^ ((~~~bx0) &&& bx1)) ⌝ ⦄ := by
  -- TODO(new-aeneas): broken inline proof; see PrcLift.lean macro TODOs.
  sorry
