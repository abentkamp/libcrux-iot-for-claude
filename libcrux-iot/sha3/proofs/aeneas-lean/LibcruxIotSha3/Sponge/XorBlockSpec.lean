...
`sponge.xor_block_into_state` calls `core.array.from_fn` directly
...
theorem xor_block_into_state_closure_call_mut_spec
  ...
  · -- True branch: k < rate/8.
    ...
    have h_try_triple := core_models_array_try_from_slice_spec
      ...
      core.U8.Insts.CoreMarkerCopy s1 h_s1_val_len
    have h_try_eq :
        core.Array.Insts.CoreConvertTryFromShared0SliceTryFromSliceError.try_from
          (8#usize : Std.Usize) core.U8.Insts.CoreMarkerCopy s1
    ...
    have h_body_eq :
      ...
      unfold core.result.Result.unwrap; simp only [bind_tc_ok]
      -- from_le_bytes is `pure (Std.core.num.U64.from_le_bytes a)`.
      have h_fle : (core.num.U64.from_le_bytes
