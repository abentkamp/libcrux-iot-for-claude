...
theorem lane.Lane2U32.interleave_spec (self : lane.Lane2U32) :
  ...
  -- TODO(new-aeneas): the old proof unfolded
  -- `libcrux_secrets.{U32,U64}.Insts.Libcrux_secretsIntCastOps.{as_u64,as_u32}`,
  -- which lived in the old `Missing.lean`. They are now provided by
  -- `LibcruxIotSha3.Extraction.FunsExternal` (no `libcrux_secrets.` prefix).
  sorry
