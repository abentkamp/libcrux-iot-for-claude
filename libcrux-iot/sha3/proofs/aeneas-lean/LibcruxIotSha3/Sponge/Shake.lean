...
/-! ### Helper: `core.slice.Slice.len` Result-level equation. -/

private theorem slice_len_eq_sh (s : Slice Std.U8) :
    core.slice.Slice.len s = .ok (Std.Slice.len s) := by
...
private theorem lift_cast_U32_MAX :
    (Std.lift (Std.UScalar.cast .Usize core.num.U32.MAX)
      : Result Std.Usize)
  ...
  -- Goal: core.num.U32.MAX.val = 4294967295#usize.val.
  show core.num.U32.MAX.val = (4294967295#usize : Std.Usize).val
  simp only [core.num.U32.MAX]
  rfl
...
theorem sha224_ema_spec
  ...
  have h_slice_len_payload :
      core.slice.Slice.len payload = .ok (Std.Slice.len payload) :=
    slice_len_eq_sh payload
  have h_slice_len_digest :
      core.slice.Slice.len digest = .ok (Std.Slice.len digest) :=
