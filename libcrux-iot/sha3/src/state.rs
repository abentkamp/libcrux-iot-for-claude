...
///
/// * The spec stores lane `A[x, y]` at flat index `5*y + x` (FIPS 202
///   §3.1.2; see `hacspec_sha3::keccak_f::get`), so byte-lane `l` lives
///   directly at `state[l]`.
...
/// `T(l) = 5 * (l % 5) + (l / 5)`, which is its own inverse. Spec index `l`
/// corresponds to impl index `T(l)`, so both helpers map through `T`.
#[cfg(test)]
pub(crate) mod cross_spec {
    ...
    /// Transpose between spec and impl flat lane indices (an involution).
